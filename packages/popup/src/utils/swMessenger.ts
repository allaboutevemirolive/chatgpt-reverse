...
const CONNECTION_ERROR_MESSAGE = "Could not connect to the extension's background service. It might be inactive. Please try again shortly.";
 ...
 */
export const sendMessageToSW: SendMessageToSW = async <T>(message: { type: string; payload?: any }): Promise<T> => {
    console.log(
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            const response: SWResponse<T> | undefined = await chrome.runtime.sendMessage(message);

            // --- Standard Response Handling ---

            // 1. Check chrome.runtime.lastError (most reliable for connection errors)
            if (chrome.runtime.lastError) {
                const lastErrorMessage = chrome.runtime.lastError.message ?? "Unknown runtime error";
                console.warn(
                ...
                if (
                    ...
                    if (attempt < MAX_RETRIES) {
                        console.warn(`Popup Helper: Connection error (lastError) on attempt ${attempt + 1}. Retrying after ${RETRY_DELAY_MS}ms...`);
                ...
                throw new Error(`Service worker communication error: ${lastErrorMessage}`);
            }
            ...
            if (response === undefined) {
                ...
                if (attempt < MAX_RETRIES) {
                    console.warn(`Popup Helper: Undefined response on attempt ${attempt + 1}. Retrying after ${RETRY_DELAY_MS}ms...`);
                    await delay(RETRY_DELAY_MS);
                    continue; // Treat as potentially recoverable for retry
                }
                throw new Error("Received undefined response from service worker after retries. Service worker might have crashed.");
            }
            ...
            if (response.success) {
                ...
                const error = new Error(response.error?.message || `Unknown error from service worker for ${message.type}.`);
                ...
                console.error(`Popup Helper: Error reported by Service Worker (Attempt ${attempt + 1}/${MAX_RETRIES + 1}) for ${message.type}:`, error);
            ...
            if (
                ...
                console.warn(`Popup Helper: Connection error caught on attempt ${attempt + 1}. Retrying after ${RETRY_DELAY_MS}ms...`);
                await delay(RETRY_DELAY_MS);
    ...
    console.error(`Popup Helper: Service worker communication failed unexpectedly for message type "${message.type}" after all retries.`);
    throw new Error(`Service worker communication failed for message type "${message.type}" after all retries.`);
};
